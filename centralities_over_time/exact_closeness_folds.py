...
# ------------------------------------------------------------------
# PARAMETERS
METRIC = "revenue"  # select metric
INPUT_DIR = "folds"
INPUT_PATTERN = "movies_*.csv"
OUTPUT_DIR = METRIC
...
# ------------------------------------------------------------------
# EXACT CLOSENESS + PARALLEL
def exact_weighted_closeness_worker(nodes, g, metric):
    weights = np.array(g.es[metric], dtype=float)
    inv_weights = np.reciprocal(weights)

    closeness = []
    for v in nodes:
        dists = np.array(g.distances(source=v, weights=inv_weights)[0])
        finite = dists[dists > 0]
        if finite.size > 0:
            c = (len(dists) - 1) / finite.sum()
        else:
            c = 0.0
        closeness.append(c)
    return closeness
...
def compute_exact_closeness_parallel(g, metric):
    ...
    with mp.Pool(n_workers) as pool:
        results = pool.starmap(
            exact_weighted_closeness_worker,
...
if __name__ == "__main__":
    dprint("Starting multi-fold exact closeness computation")

    input_files = sorted(glob.glob(os.path.join(INPUT_DIR, INPUT_PATTERN)))

    for csv_file in input_files:
        fold_name = os.path.splitext(os.path.basename(csv_file))[0].replace(
            "movies_", ""
        )

        dprint(f"Processing fold {fold_name}")

        df = load_and_filter_data(csv_file)
        if df.empty:
            dprint("No data after filtering — skipping")
            continue

        G = build_multigraph(df)

        dprint("Computing exact weighted closeness")
        t0 = time.time()
        closeness = compute_exact_closeness_parallel(G, METRIC)
        dprint(f"Computed in {time.time() - t0:.2f}s")
        ...
        output_file = os.path.join(
            OUTPUT_DIR,
            f"exact_closeness_{fold_name}.csv"
        )
        result.to_csv(output_file, index=False)

        dprint(f"Saved {output_file}")

    dprint("All folds processed.")
