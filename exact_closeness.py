...
OUTPUT_DIR = "closeness_all_metrics"
...
if __name__ == "__main__":
    dprint(f"Starting exact closeness computation for {INPUT_FILE}")
    ...
    for metric in METRICS:
        dprint(f"Computing exact closeness for metric '{metric}'")
        t0 = time.time()
        closeness = compute_exact_closeness_parallel(G, metric)
        result_df[f"closeness_{metric}"] = closeness
        dprint(f"Done {metric} in {time.time() - t0:.2f}s")

    # sort by co and save
    result_df = result_df.sort_values("closeness_co", ascending=False)
    base_name = os.path.splitext(os.path.basename(INPUT_FILE))[0]
    output_file = os.path.join(OUTPUT_DIR, f"exact_closeness_all_metrics_{base_name}.csv")
